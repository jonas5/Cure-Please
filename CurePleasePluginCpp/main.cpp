...
private:
    ...
    void PipeThread()
    {
        ...
        while (!m_Shutdown) {
            m_LogManager->Log(LogLevel::Info, "Miraculix", "Pipe thread waiting for connection...");
            m_hPipe = CreateNamedPipeW(PipeName.c_str(), PIPE_ACCESS_DUPLEX,
                PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT,
                1, 1024, 1024, 0, NULL);

            if (m_hPipe == INVALID_HANDLE_VALUE) {
                m_LogManager->Log(LogLevel::Error, "Miraculix", "Failed to create named pipe.");
                Sleep(5000);
                continue;
            }
            ...
            if (connected) {
                m_LogManager->Log(LogLevel::Info, "Miraculix", "Pipe client connected.");
                {
                    std::lock_guard<std::mutex> lock(m_PipeMutex);
                ...
                while (!m_Shutdown) {
                    if (ReadFile(m_hPipe, buffer, sizeof(buffer) - 1, &bytesRead, NULL)) {
                        ...
                        m_LogManager->Log(LogLevel::Info, "Miraculix", ("Received from pipe: " + received).c_str());
                        ...
                        if (GetLastError() != ERROR_IO_PENDING) {
                            m_LogManager->Log(LogLevel::Error, "Miraculix", "ReadFile error or pipe closed.");
            ...
            m_LogManager->Log(LogLevel::Info, "Miraculix", "Pipe client disconnected.");
...
public:
    ...
    {
        m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Incoming packet ID: " + std::to_string(id)).c_str());
        ...
        if (id == 0x00A && size >= 4)
        {
            if (!m_isZoning) {
                m_isZoning = true;
                m_spellTimers.clear();
                m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", "[Miraculix] Zoning detected, clearing all spell timers.");
        ...
        if (id == 0x29 && size >= 16)
        {
            ...
            m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Action packet: " + actorName + " used ability " + std::to_string(abilityId)).c_str());
            ...
            if (!actorName.empty() && actorName != "Unknown" && actorName != "None")
            {
                ...
                {
                    m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DISPEL_REQUESTED: " + actorName + " used " + it_def->second).c_str());
                ...
                {
                    m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DISPEL_REQUESTED: " + actorName + " used " + it_mag->second).c_str());
                ...
                {
                    m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DISPEL_REQUESTED: " + actorName + " used " + it_eva->second).c_str());
        ...
        if (id == 0x28 && size >= 32)
        {
            ...
            m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Spell action packet, category: " + std::to_string(category)).c_str());
            ...
            if (category == 4)
            {
                ...
                m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Magic finish: " + actorName + " cast " + spellName).c_str());
                ...
                if (it != spells.end())
                {
                    ...
                    for (uint8_t i = 0; i < targetCount; ++i)
                    {
                        ...
                        if (spell.cooldown > 0)
                        {
                            ...
                            m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] BUFF_APPLIED: " + spell.name + " on " + targetName).c_str());
                ...
                m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", "[Miraculix] CAST_FINISH");
            ...
            {
                if (param == 24931) {
                    m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", "[Miraculix] CAST_BLOCKED");
                    WriteToPipe("CAST_BLOCKED\n");
                }
                else if (param == 28787) {
                    m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", "[Miraculix] CAST_INTERRUPT");
                ...
                else if (param == 258) // Magic is resisted.
                {
                    ...
                    {
                        m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DEBUFF_RESISTED: " + spellName + " on " + targetName).c_str());
        ...
        else if (id == 0x00E && size > 4)
        {
            ...
            if (pos_afflict != std::string::npos)
            {
                ...
                m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DEBUFF_APPLIED: " + debuff_name + " on " + player_name).c_str());
            ...
            if (pos_wears_off1 != std::string::npos && pos_wears_off2 != std::string::npos && pos_wears_off1 < pos_wears_off2)
            {
                ...
                if (is_rdm_debuff) {
                    m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DEBUFF_FADED: " + spell_name + " from " + player_name).c_str());
                    ...
                    if (is_buff) {
                        m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] BUFF_FADED: " + spell_name + " from " + player_name).c_str());
                        WriteToPipe("BUFF_FADED|" + player_name + "|" + spell_name + "\n");
                        m_spellTimers[player_name].erase(spell_name);
                    }
                    else {
                        m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] DEBUFF_FADED (non-RDM): " + spell_name + " from " + player_name).c_str());
                        WriteToPipe("DEBUFF_FADED|" + player_name + "|" + spell_name + "\n");
                    }
                }
                return false;
    ...
    bool HandleOutgoingPacket(uint16_t id, uint32_t size, const uint8_t* data, uint8_t* modified,
        ...
        m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Outgoing packet ID: " + std::to_string(id)).c_str());
        ...
        if (id == 0x15 && size >= 8) {
            ...
            m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Action start packet, category: " + std::to_string(category)).c_str());

            // Category 8: Magic spell
            if (category == 8) {
                std::string spellName = ResolveSpellName(spellId);
                m_LogManager->Log(Ashita::LogLevel::Info, "Miraculix", ("[Miraculix] Player starts casting " + spellName).c_str());
                auto it = spells.find(spellId);
                if (it != spells.end()) {
                        ...
                        if (!targetName.empty() && targetName != "Unknown" && targetName != "None") {
                            ...
                            if (m_spellTimers[targetName].count(spell_info.name)) {
                                ...
                                if (elapsed < spell_info.cooldown) {
                                    m_LogManager->Log(Ashita::LogLevel::Warning, "Miraculix", ("[Miraculix] Blocking cast of " + spell_info.name + " on " + targetName + " due to cooldown.").c_str());
